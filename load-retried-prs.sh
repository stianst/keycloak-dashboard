...
for i in $(gh api -X GET repos/keycloak/keycloak/actions/workflows/ci.yml/runs -f per_page=100 -f status=success -f event=pull_request | jq -r '.workflow_runs[] | [.id, .run_attempt, .created_at] | @csv'); do
    ID=$(echo $i | cut -d ',' -f 1)
    ...
    if [ "$RUN_ATTEMPTS" -gt 1 ]; then
        ...
        while [ $RUN_ATTEMPT -lt $RUN_ATTEMPTS ]; do
            ...
            if [ "$CONCLUSION" == "failure" ]; then
                echo "$CREATED_AT,https://github.com/keycloak/keycloak/actions/runs/$ID/attempts/$RUN_ATTEMPT"
            fi
