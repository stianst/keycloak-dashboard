...
public class Bugs {
    ...
    public Bugs(GitHubData data, Teams teams) {
        ...
        int createdLast7Days = (int) issues.stream().filter(i -> i.getCreatedAt().after(Date.MINUS_8_DAYS)).count();
        int closedLast7Days = (int) issues.stream().filter(i -> i.getClosedAt() != null && i.getClosedAt().after(Date.MINUS_8_DAYS)).count();
        int createdLast30Days = (int) issues.stream().filter(i -> i.getCreatedAt().after(Date.MINUS_31_DAYS)).count();
        int closedLast30Days = (int) issues.stream().filter(i -> i.getClosedAt() != null && i.getClosedAt().after(Date.MINUS_31_DAYS)).count();
        int createdLast90Days = (int) issues.stream().filter(i -> i.getCreatedAt().after(Date.MINUS_91_DAYS)).count();
        int closedLast90Days = (int) issues.stream().filter(i -> i.getClosedAt() != null && i.getClosedAt().after(Date.MINUS_91_DAYS)).count();
        ...
        stats.add(new BugStat("Created last 7 days", createdLast7Days, createdLast7Days > closedLast7Days ? 0 : 999, "label:kind/bug created:>" + Date.MINUS_8_DAYS_STRING));
        stats.add(new BugStat("Closed last 7 days", closedLast7Days, createdLast7Days > closedLast7Days ? 0 : 999, "is:closed label:kind/bug closed:>" + Date.MINUS_8_DAYS_STRING));

        stats.add(new BugStat("Created last 30 days", createdLast30Days, createdLast30Days > closedLast30Days ? 0 : 999, "label:kind/bug created:>" + Date.MINUS_31_DAYS_STRING));
        stats.add(new BugStat("Closed last 30 days", closedLast30Days, createdLast30Days > closedLast30Days ? 0 : 999, "is:closed label:kind/bug closed:>" + Date.MINUS_31_DAYS_STRING));

        stats.add(new BugStat("Created last 90 days", createdLast90Days, createdLast90Days > closedLast90Days ? 0 : 999, "label:kind/bug created:>" + Date.MINUS_91_DAYS_STRING));
        stats.add(new BugStat("Closed last 90 days", closedLast90Days, createdLast90Days > closedLast90Days ? 0 : 999, "is:closed label:kind/bug closed:>" + Date.MINUS_91_DAYS_STRING));
